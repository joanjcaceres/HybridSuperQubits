...
def fit_fluxonium2(transition_data, bounds):
    """
    Fits the Fluxonium model based on the provided transitions.

    Parameters:
    -----------
    transition_data : list of tuples
        Each tuple contains:
        - data (ndarray): Numpy array containing the transition data with columns 'phi' and 'e_ij'.
    ...
    --------
    transition_data = [
        (np.array([[phi_values], [e_ij_values]]), 0, 1),  # transition from 0 to 1
        (np.array([[phi_values], [e_ij_values]]), 1, 2)   # transition from 1 to 2
    ]
    ...
    fluxonium_fit, result = fit_fluxonium(transition_data, bounds)
    """
    
    transitions = []
    
    for data, i, j in transition_data:
        df = pd.DataFrame(data, columns=['phi', 'e_ij'])
