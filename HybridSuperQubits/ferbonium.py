...
class Ferbonium(QubitBase):
    ...
    def __init__(self, Ec, El, Ej, Gamma, delta_Gamma, er, phase, dimension, flux_grouping: str = 'ABS', Delta = 40):
        """
    ...
    def jrl_potential(self) -> np.ndarray:
            ...
            return - self.Gamma * np.kron(sigma_z(), cosm(phase_op/2)) \
                ...
                - self.Ej * np.kron(np.eye(2), cosm(phase_op))
    ...
    def d_hamiltonian_d_phase(self) -> np.ndarray:
        ...
        if self.flux_grouping == 'L':
            ...
            + self.Ej * np.kron(np.eye(2), sinm(phase_op))
    ...
    def potential(self, phi: Union[float, np.ndarray]) -> np.ndarray:
        ...
        for i, phi_val in enumerate(phi_array):
            if self.flux_grouping == 'ABS':
                ...
                - self.Ej * np.eye(2) * np.cos((phi_val + self.phase))
                ...
                - self.Ej * np.eye(2) * np.cos(phi_val + phi_ext)
    ...
    def plot_wavefunction(
        ...
        **kwargs
        ...
        offset : float, optional
            Offset for the wavefunction and potential (default is 0).
        basis: str, optional
            Basis in which to return the wavefunction ('phase' or 'charge') (default is 'phase').
        rotate : bool, optional
            Whether to rotate the basis (default is False).
        ...
        **kwargs
        ...
        if plot_potential:
            ...
            ax.plot(phi_grid, potential[:, 0] + offset, color='black', label='Potential')
            ax.plot(phi_grid, potential[:, 1] + offset, color='black')
        ...
        for idx in which:
            ...
            ax.plot(
                phi_basis_labels,
                wavefunc_energy + offset + scaling * y_values,
            ...
            ax.plot(
                phi_basis_labels, 
                wavefunc_energy + offset + scaling * y_values_down,
