...
from fluxonium import calculate_CQPS_rate
...
Q_cap_r = 30 * 1e3 # Obtained from the T1 from FBW10S4
totalIslandsLength = 68 #µm
fluxLineCapacitance = 0 #fF #It can be zero if we use a coil instead of a flux line.
...
class FluxoniumManager():
    def __init__(
        ...
        self.flux = flux
        self.cutoff = cutoff
        self.original_bounds = None # TODO: Remove it.

    def fluxonium_creator(self, small_jj_area, n_junctions, flux, **kwargs):
        """
        ...
        ----------
        small_jj_area: float
            The area of the small Josephson junction in µm².
        n_junctions: int
            The number of Josephson junctions.
        flux : float
    ...
    def _Gamma2(self,params_normalized):
        """
        ...
        -------
        float
            The calculated decoherence rate Gamma (by default in units of GHz).
        """
        ...
        fluxonium = self.fluxonium_creator(small_jj_area, n_junctions, flux)
        
        Gamma2 = fluxonium.t2_effective(
                            noise_channels=[
                                ('t1_capacitive', dict(Q_cap = Q_cap_r * (6/fluxonium.E01())**0.2)),
    ...
    def minimizer(self, bounds:dict):
        ...
        self.optimal_params = self._denormalize_params(self.result.x)
        self.T2 = 1/(self.result.fun * 1e9) # units: s.
        
        if len(self.optimal_params) == 2:
            small_jj_area, n_junctions = self.optimal_params
            flux = self.flux
        elif len(self.optimal_params) == 3:
            small_jj_area, n_junctions, flux = self.optimal_params
        
        self.optimal_fluxonium = self.fluxonium_creator(small_jj_area, n_junctions, flux)
